...
<html>
    ...
    <body>
                        ...
                        <div class="tab-content profile-tab" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <input type="date" name="licence_expiration" id="licence_expiration" class="form-control licence_number" 
                                               min="<?php echo date('Y-m-d', strtotime('+3 years')); ?>"
                                               max="<?php echo date('Y-m-d', strtotime('+17 years')); ?>"
                                               maxlength="15" required>
                                        <p id="licence_expiration_error" style="color: red;"></p>
                                    </div>
                                </div><br/>

                                <script>
                                    document.addEventListener("DOMContentLoaded", function () {
                                        var licenceExpirationInput = document.getElementById("licence_expiration");
                                        var errorElement = document.getElementById("licence_expiration_error");
                                        var currentDate = new Date();
                                        var minYear = currentDate.getFullYear() + 3; // Minimum year (2023)
                                        var maxYear = 2040; // Maximum year
                                        licenceExpirationInput.addEventListener("input", function () {
                                            var inputValue = licenceExpirationInput.value;
                                            var firstTwoChars = inputValue.substr(0, 2);
                                            var remainingChars = inputValue.substr(2);

                                            if (!/^[A-Z]{2}$/.test(firstTwoChars) || !/^\d+$/.test(remainingChars)) {
                                                errorElement.textContent = "Invalid format.";
                                                licenceExpirationInput.setCustomValidity("Invalid format");
                                            } else {
                                                errorElement.textContent = "";
                                                licenceExpirationInput.setCustomValidity("");
                                            }
                                            var selectedDate = new Date(licenceExpirationInput.value);
                                            var selectedYear = selectedDate.getFullYear();

                                            if (selectedYear < minYear || selectedYear > maxYear) {
                                                errorElement.textContent = "Invalid year. Please enter a year between 2023 and 2040.";
                                                licenceExpirationInput.setCustomValidity("Invalid year");
                                            } else {
                                                errorElement.textContent = "";
                                                licenceExpirationInput.setCustomValidity("");
                                            }
                                    ...
                                    $(document).ready(function () {
                                        $("#licence_number").keyup(function (e) {
                                            ...
                                            for (var i = 0; i < dInput.length; i++) {
                                                ...
                                                if (validstr.match(/^\d/) && validstr.match(/^\d/)[0] === '0') {
                                                    $("#ErrorR_no").html("Number cannot start with all zeros").show();
                                                    return false;
                                                }
                                ...
                                <script>
                                    $(document).ready(function () {
                                        $('#MobileNo').on('keypress', function (e) {
                                            ...
                                            if (regex.test(str)) {
                                                // Allow input if it's the first character and not 1, 2, 3, or 4
                                                if ($this.val().length === 0 && ['1', '2', '3', '4' ,'5'].includes(str)) {
                                                    e.preventDefault();
                                                    return false;
                                                }
                                                return true;
        ...
        if (isset($_POST['AddCustomer'])) {
            ...
            $customer = $conn->prepare("INSERT INTO customer(Name,Email,Password,Mobile,Date_Of_Birth,Driving_Licence,AadharCard,Image,Registration_Date, Expiration_date) VALUES (?,?,?,?,?,?,?,?,?,?)");
            $customer->bind_param("ssssssssss", $fullname, $Emailid, $password, $mobile, $DOB, $licence_number, $Aadharcard, $imgname, $Registration_Date,$expiration);
